...
const Header = () => {
    ...
    const isStudentLoginPage = pathname.includes("student")
    return <div className={`w-full flex justify-between px-10 md:px-40 py-5`}>
        ...
        <div className={`flex items-center gap-x-3 md:gap-x-5`}>
            ...
            <Button variant={`outline`}>
                <Link className={`text-primary text-sm md:text-base`} href={`${isLoginPage ? "/signup" : "/login"}`}>{isLoginPage ? "Đăng ký" : "Đăng nhập"}</Link>
            </Button>
            <Button variant={`default`} className={`${pathname.includes("login/student") || pathname.includes("login/teacher")  ? "block" : "hidden"}`}>
                <Link className={`text-secondary text-sm md:text-base`} href={`${isStudentLoginPage ? "teacher" : "student"}`}>{isStudentLoginPage ? "Tôi là giáo viên" : "Tôi là học sinh"}</Link>
            </Button>
        </div>
    </div>
}
